...
{ 
  'use strict';
    dataEntryClient.Views.CampaignTagsView = Backbone.View.extend({
      length_tags: 0,
    	el: "#page",
    	...
    	initialize: function(attrs) {
      	...
      	this.id = options.id;
            ...
            //debugger
    				that.$el.html(that.template({obj: response.campaign_tags, length_tags: response.meta.length_tags, agent: response.agent, text_applicable_promo_message: response.not_applicable_promo_message}));
    				compile_sidebar({
                  		id: that.id,
