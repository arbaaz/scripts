    ...
    var DeliveryAreaView = Backbone.View.extend({
      ...
      back_to_normal_view: function () {
        _.each(window.polygons, function (polygon) {
          ...
          window.map_polygons[polygon.get('id')].setOptions({
      ...
      display_heat_map: function (min_del_amt_mapping, category) {
        ...
        var sorted_minimum_amount = _.sortBy(min_del_amt_mapping, function (da) {
          return da[category];
        });
        var unique = 1;
        ...
        for (i = 0; i < sorted_minimum_amount.length - 1; i++) {
          window.map_polygons[sorted_minimum_amount[i].id].setOptions({
            strokeColor: color_slots[slot],
            strokeOpacity: 0.9,
            strokeWeight: 3,
            fillOpacity: 0.3,
            zIndex: 1000,
            fillColor: color_slots[slot]
          });
      ...
      get_colors: function (slots) {
        var colors = {
          "7": ["#026320", "#127C04", "#539507", "#A6AE09", "#C6810D", "#E03C10", "#F91544"],
      ...
      new_tag: function (e) {
        ...
        var curr_g = _.filter(window.collections.groups, function (g) {
          if (g.id == parseInt(currentEl.val())) {
            return g
          }
        })[0];
      ...
      fetch_polygons: function (callback) {
        ...
        this.delivery_area_polygons.fetch({
          success: function (data, collection) {
            callback({
              polygons: data.models,
              restaurant_coordinates: data.restaurant_coordinates,
              polygons_not_shown: collection.payload.groups_not_shown
            });
          },
          error: function (data, response, error) {
