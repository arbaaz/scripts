define([
  ...
  function (_, $, Backbone) {
    'use strict';
    var TimingView = Backbone.View.extend({
      ...
      save_changes: function (e) {
        ...
        if (timings.timings.length != 0) {
          ...
          var default_timings = timings.timings.filter(function (timing) {
            return parseInt(timing.timing_type) == window.collections.timing_type.DEFAULT
          });
          var dish_timings = timings.timings.filter(function (timing) {
            return parseInt(timing.timing_type) == window.collections.timing_type.DISHES
          });
          var operational_timings = timings.timings.filter(function (timing) {
            return parseInt(timing.timing_type) == window.collections.timing_type.OPERATIONAL
          });
      ...
      is_subset_operational: function (timing_obj) {
        ...
        _.each(operational_time, function (o) {
          if (o.end_in_secs < o.start_in_secs) {
            o.end_in_secs += 86400
          }
      ...
      is_overlapping_operational: function (id) {
        ...
        var operational_object = []
        if (timing[0].end_in_secs < timing[0].start_in_secs) {
          timing[0].end_in_secs += 86400
        }
        operational_object.push(timing[0].start_in_secs, timing[0].end_in_secs)
        var remaining_timings = _.reject(timings.timings, function (time) {
          return time.timing_type == window.collections.timing_type.OPERATIONAL
        });
