...
(function () {
  ...
  dataEntryClient.Models.ItemSize = Backbone.RelationalModel.extend({
    ...
    validate_prices: function (price_collection) {
      ...
      var timings = price_collection.map(function (m) {
        return m.get('timing_id')
      });
    ...
    validate_delete: function (price_collection) {
      ...
      } else if (price_collection.filter(function (price) {
          if (price.get('id').split('_')[0] != 'new') {
            return true;
          }
        }).length < 2) {
...
(function () {
  ...
  dataEntryClient.Models.Item = Backbone.RelationalModel.extend({
    ...
    validate: function () {
      ...
      var message = "";
      var sizes = this.get('sizes').models.map(function (m) {
        return m.get('size')
      });
    ...
    validate_sizes: function (size_collection) {
      ...
      } else if (size_collection.filter(function (size) {
          if (size.get('id').split('_')[0] != 'new') {
            return true;
          }
        }).length < 2) {
